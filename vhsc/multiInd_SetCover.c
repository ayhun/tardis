...
bool conflictResFlag = false;
bool weightedHeuristicFlag = false;
...
if (listClusterEl[clusterId].oldBestIsGood==true)
{		
	int count=0;
	for ( count=0; count<totalNumInd; count++)
...
listClusterEl[clusterId].oldBestIsGood=true;
...
int removeInd(int ind, int j)
{
	...
	listClusterEl[j].oldBestIsGood=false;
}
				...
				if (listClusterEl[NotSelectedClus].indIdCount[countInd]==0 && ptr1->sup[countInd]>0 && ptr2->sup[countInd]>0)
					{
						...
						listClusterEl[NotSelectedClus].oldBestIsGood=false;
					}
	...
	while(ptrRead!=NULL && totalReadRemove>0)
	{
		if (readsToMark[ptrRead->indId]>0 && listReadEl[ptrRead->readId].readCovered==0)
		{
			...
			while (ptrClusterId!=NULL)
			{
				...
				listClusterEl[ptrClusterId->clusterId].oldBestIsGood=false;
...
maxScoreInBuffer=inf-1;
	...
	if (bestWeight<inf)
		{
			if (conflictResFlag == false || conflictsAny(bestWeightSet, bestReads)==-1)
			{
...

				{
					...
					if (conflictResFlag==true)
						addToConflict(bestWeightSet, bestReads);
	
					numCallsRequsted--;
					listClusterEl[bestWeightSet].oldBestIsGood=false;
				}
			}
		}
	else 
...
int vh_setcover(char* divetadd, char* outputread, char* outputfile, char* svfile){
	...
	// 		coverageFp=fopen(argc[count+1], "r");
	// 	if (strcmp(argc[count],"-m")==0)
	// 		fpMobile=fopen(argc[count+1],"r");
	// 	if (strcmp(argc[count],"-t")==0)
	// 		numCallsRequsted = atoi(argc[count+1]);
	// 	if (strcmp(argc[count],"-o")==0)
	// 		fpOut=fopen(argc[count+1],"w");
	// 	if (strcmp(argc[count],"-cr")==0)
	// 		conflictResFlag=true;
	// 	if (strcmp(argc[count],"-wh")==0)
	// 		weightedHeuristicFlag=true;
	// 	if (strcmp(argc[count],"-ms")==0)
	// 		mismatchPenalty=atof(argc[count+1]);
	// 	if (strcmp(argc[count],"-minSup")==0)
	// 		minimumSupNeeded=atoi(argc[count+1]);
	// }
...
for (count=0; count<argv; count++)
	{	
		if (strcmp(argc[count],"-l")==0)
			libFp=fopen(argc[count+1],"r");
		if (strcmp(argc[count],"-r")==0)
			readFp=fopen(argc[count+1],"r");
		if (strcmp(argc[count],"-c")==0)
			clusterFp=fopen(argc[count+1],"r");
		if (strcmp(argc[count],"-w")==0)
			weightsFp=fopen(argc[count+1],"r");
		if (strcmp(argc[count],"-co")==0)
			coverageFp=fopen(argc[count+1], "r");
