...
int vh_createBreakPointIntervals_Insertion (int brkPointLeft)
{
  ...
  if (g_listRightBrkPointIntrCount > 0)
      ...
      while (listRightBrkPointIntrId < g_listRightBrkPointIntrCount)
	{
	  if (g_listRightBrkPointIntr
	      [listRightBrkPointIntrId].readMappingPtr->locMapRightStart ==
	      brkPointLeft)
	    {
  ...
  while (ptrMappingOnGenome != NULL)
	...
	ptrMappingOnGenome->readMappingPtr->libInfo->maxDelta -
	(ptrMappingOnGenome->readMappingPtr->locMapRightStart -
	 ptrMappingOnGenome->readMappingPtr->locMapLeftEnd);
      locBrkPointLeftTemp =
	ptrMappingOnGenome->readMappingPtr->libInfo->minDelta -
	(ptrMappingOnGenome->readMappingPtr->locMapRightStart -
	 ptrMappingOnGenome->readMappingPtr->locMapLeftEnd);
...
int vh_createInsertionClusters (int chroSize)
{
