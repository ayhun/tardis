...
int vh_addToGenomeIndex_Inversion (char *chroName)
{
  LibraryInfo *libInfo;
  ...
  while (libInfo != NULL)
      ...
      while (divetReadMappingPtr != NULL)
	{
	  ...
	  if (strcmp (divetReadMappingPtr->chroName, chroName) == 0
		...
		&& divetReadMappingPtr->orientationRight == 'F')
	       || (divetReadMappingPtr->orientationLeft == 'R'
		   && divetReadMappingPtr->orientationRight == 'R'))
	      && (divetReadMappingPtr->locMapLeftEnd <
		  divetReadMappingPtr->locMapRightStart)
	      && (divetReadMappingPtr->locMapRightStart -
		  divetReadMappingPtr->locMapLeftEnd < maxInversionLen))
	    {
		...
		{
		  leftWindowStart =
		    vh_max (divetReadMappingPtr->locMapLeftStart -
			 libInfo->maxDelta, 0);
		  newEl->next = g_genomeIndexStart[leftWindowStart];
		  newEl2->next =
		    g_genomeIndexEnd[divetReadMappingPtr->locMapLeftStart];
...
int vh_reevaluate_rightBrkPoint_FF (int id, int brkPointLeft)
{
...
int vh_reevaluate_rightBrkPoint_RR (int id, int brkPointLeft)
{
  int brkRightTemp, brkLeftTemp;
  brkRightTemp =
  ...
  if (g_listRightBrkPointIntrCount > 0)
      ...
      while (listRightBrkPointIntrId < g_listRightBrkPointIntrCount)
	{
	  ...
	  if (g_listRightBrkPointIntr
	      [listRightBrkPointIntrId].readMappingPtr->orientationLeft == 'F'
	      &&
	      g_listRightBrkPointIntr
	      [listRightBrkPointIntrId].readMappingPtr->orientationRight ==
	      'F')
	    {
	      if ((g_listRightBrkPointIntr
		   [listRightBrkPointIntrId].readMappingPtr->locMapLeftEnd +
		   g_listRightBrkPointIntr
		   [listRightBrkPointIntrId].readMappingPtr->libInfo->
		   maxDelta == brkPointLeft)
		  || (brkPointLeft ==
		      g_listRightBrkPointIntr
		      [listRightBrkPointIntrId].readMappingPtr->
		      locMapRightEnd))
		{
	      ...
	      else
		{
		  ...
		  vh_reevaluate_rightBrkPoint_FF (tempListRightBrkPointIntrId,
					       brkPointLeft);
		  listRightBrkPointIntrId++;
	    ...
	    if (g_listRightBrkPointIntr
		[listRightBrkPointIntrId].readMappingPtr->orientationLeft ==
		'R'
		&&
		g_listRightBrkPointIntr
		[listRightBrkPointIntrId].readMappingPtr->orientationRight ==
		'R')
	    {
	      if ((g_listRightBrkPointIntr
		   [listRightBrkPointIntrId].readMappingPtr->
		   locMapLeftStart == brkPointLeft)
		  ||
		  (g_listRightBrkPointIntr
		   [listRightBrkPointIntrId].readMappingPtr->
		   locMapRightStart - brkPointLeft <
		   g_listRightBrkPointIntr
		   [listRightBrkPointIntrId].readMappingPtr->libInfo->
		   minDelta))
		{
	      ...
	      else
		{
		  ...
		  vh_reevaluate_rightBrkPoint_RR (tempListRightBrkPointIntrId,
					       brkPointLeft);
		  listRightBrkPointIntrId++;
		  tempListRightBrkPointIntrId++;
		}
  ...
  while (ptrMappingOnGenome != NULL)
    {
      ...
      if (ptrMappingOnGenome->readMappingPtr->orientationLeft == 'F'
	  && ptrMappingOnGenome->readMappingPtr->orientationRight == 'F')
	{
	...
	{
	  locBrkPointRightTemp =
	    vh_max (ptrMappingOnGenome->readMappingPtr->locMapRightStart -
		 vh_max (ptrMappingOnGenome->readMappingPtr->libInfo->minDelta -
...
int vh_createInversionClusters (int chroSize)
{
