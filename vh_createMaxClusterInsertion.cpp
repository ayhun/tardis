...
int addToGenomeIndex_Insertion(char *chroName)
{
	LibraryInfo* libInfo;
	...
	while (libInfo!=NULL)
	{
		...
		while(divetReadMappingPtr!=NULL)
		{
			...
			if (strcmp(divetReadMappingPtr->chroName, chroName)==0 && divetReadMappingPtr->orientationLeft=='F' && divetReadMappingPtr->orientationRight=='R' && (divetReadMappingPtr->locMapRightStart - divetReadMappingPtr->locMapLeftEnd < libInfo->minDelta) && noGap(chroName, divetReadMappingPtr->locMapLeftEnd, divetReadMappingPtr->locMapRightStart) && (divetReadMappingPtr->locMapRightStart > divetReadMappingPtr->locMapLeftEnd)) 
			{
...
void initializeReadMapping_Insertion(char* chroName, int chroSize)
{
	//Gap info is global	
	...
	addToGenomeIndex_Insertion(chroName);
...
bool createBreakPointIntervals_Insertion(int brkPointLeft)
{
	...
	{
		newElAdded=true;
		locBrkPointRightTemp = ptrMappingOnGenome->readMappingPtr->libInfo->maxDelta - (ptrMappingOnGenome->readMappingPtr->locMapRightStart - ptrMappingOnGenome->readMappingPtr->locMapLeftEnd);
		locBrkPointLeftTemp = ptrMappingOnGenome->readMappingPtr->libInfo->minDelta - (ptrMappingOnGenome->readMappingPtr->locMapRightStart - ptrMappingOnGenome->readMappingPtr->locMapLeftEnd);
...
int createInsertionClusters(int chroSize) 
{
	...
	{
		newElAdded=false;
		newElAdded = createBreakPointIntervals_Insertion(leftBreakPoint);		
		if (newElAdded)
			createIntersectingIntervals(leftBreakPoint,1);
	}
	flushOut(g_listPotClusterFound, leftBreakPoint, 1);
}
